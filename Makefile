...
gateway: docker
	docker run --network gateway --restart unless-stopped -p 80:80 -p 443:443 -v /var/log/nginx:/var/log/nginx:rw -e NGINX_ENVSUBST_OUTPUT_DIR=/etc/nginx -it -d fractalnetworks/selfhosted-gateway:latest
